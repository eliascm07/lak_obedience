...
def _patch_urdf_for_mujoco(urdf_path: str) -> str:
    ...
    """
    # urdf_dir = str(Path(urdf_path).parent.parent)  # directorio del paquete
    ...
    for elem in root.iter():
        for attr in ["filename"]:
            ...
            if val.startswith("package://"):
                # package://lak_description/meshes/... → /abs/path/meshes/...
                # rel = val.replace("package://lak_description/", "")
                # abs_path = os.path.join(urdf_dir, "lak_description", rel)
...
class MujocoSimulator:
    ...
    def reset(self, q_pin: np.ndarray):
        ...
        # Base position y orientación
        self.mj_data.qpos[:3]  = q_pin[:3]
        # MuJoCo usa wxyz, Pinocchio usa xyzw → reordenar
        self.mj_data.qpos[3]   = q_pin[6]   # w
        self.mj_data.qpos[4:7] = q_pin[3:6] # xyz
    ...
    def apply_position_control(self, q_des_pin: np.ndarray,
        ...
        """
        Aplica control de posición articular (PD) en los joints actuados.

        Parameters
        ----------
        q_des_pin : ndarray (nq,)
            Configuración deseada en formato Pinocchio.
        kp, kd : float
            Ganancias proporcional y derivativa del controlador PD.
        """
        for pin_idx, mj_idx in self._pin_to_mj_idx.items():
            q_actual  = self.mj_data.qpos[mj_idx]
            dq_actual = self.mj_data.qvel[mj_idx - 7] if mj_idx >= 7 else 0.0
            q_target  = q_des_pin[pin_idx]
            torque = kp * (q_target - q_actual) - kd * dq_actual
            # Clamp al límite de torque del joint (effort=1 en el URDF)
            self.mj_data.ctrl[mj_idx - 7] = np.clip(torque, -1.0, 1.0)
    ...
    def get_q(self) -> np.ndarray:
        ...
        """
        nq = self.lak.model.nq
        q = np.zeros(nq)

        # Base
        q[:3]  = self.mj_data.qpos[:3]
        q[3:6] = self.mj_data.qpos[4:7]  # xyz de quaternión
        q[6]   = self.mj_data.qpos[3]    # w de quaternión
